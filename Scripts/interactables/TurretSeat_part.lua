...
TurretSeat2 = class()
TurretSeat2.poseWeightCount = 3
TurretSeat2.ammoTypes = {
    {
        name = "AA Rounds",
        damage = 100,
        velocity = 300,
...
TurretSeat2.overrideAmmoTypes = {}
TurretSeat2.containerToAmmoType = {
...
TurretSeat2.baseUUID = "e4497545-5f77-4d59-bfbf-ce5692284322"

function TurretSeat2:server_onCreate()
    self.shotCounter = 0
...
function TurretSeat2:sv_syncToLateJoiner(player)
    self.network:sendToClient(player, "cl_syncToLateJoiner", { self.base, self.ammoType })
end

function TurretSeat2:server_onFixedUpdate()
    if not sm.exists(self.base) then return end

    local col = self.base.shape.color
...
function TurretSeat2:sv_OnPlayerDeath(player)
    self:sv_OnPlayerSuddenUnSeated()
    self.network:sendToClient(player, "cl_unSeat_graphics")
end

function TurretSeat2:sv_OnPlayerSuddenUnSeated() end

function TurretSeat2:server_onUnload()
    if not sm.exists(self.base) then return end
    sm.event.sendToInteractable(self.base, "sv_respawnSeat")
end

---@param player Player
function TurretSeat2:server_onRemoved(player)
...
function TurretSeat2:sv_seatRespawn(player)
    self:sv_seat(nil, player)
    self.network:sendToClient(player, "cl_seat_partial")
end

---@param caller Player
function TurretSeat2:sv_seat(args, caller)
    local char = caller.character
    self.interactable:setSeatCharacter(char)
    self.sv_seated = char

    caller.publicData = caller.publicData or {}
    caller.publicData.turretSeat = self.interactable
end

---@param caller Player
function TurretSeat2:sv_unSeat(args, caller)
    --self:sv_toggleLight(false)

    self.sv_seated = nil
    self.interactable:setSeatCharacter(caller.character)
    ...
    caller:setCharacter(
        sm.character.createCharacter(
            ...
            self.shape.worldPosition + rot * (-vec3_up * 0.15 + vec3_forward),
            yaw, pitch
        )
    )
...
---@param player Player
function TurretSeat2:sv_unSeat_event(player)
    self:sv_unSeat(nil, player)
    self.network:sendToClient(player, "cl_unSeat_graphics")
end

function TurretSeat2:server_onProjectile(position, airTime, velocity, projectileName, shooter, damage, customData, normal, uuid)
    sm.event.sendToInteractable(self.base, "sv_takeDamage", damage)
end

function TurretSeat2:server_onMelee(position, attacker, damage, power, direction, normal)
    sm.event.sendToInteractable(self.base, "sv_takeDamage", damage)
end

function TurretSeat2:server_onExplosion(center, destructionLevel)
    sm.event.sendToInteractable(self.base, "sv_takeDamage", destructionLevel * 25)
end

function TurretSeat2:server_canErase()
    return self.interactable.publicData.health >= self.base.publicData.maxHealth and self.sv_seated == nil
end

function TurretSeat2:sv_updateAmmoType(ammoType)
...
function TurretSeat2:sv_shoot(ammoType, caller)
    if not self.sv_controlsEnabled then return end
    ...
    local rot = self.shape.worldRotation
    local hit, result = sm.physics.spherecast(startPos, endPos, 0.1, self.shape, rayFilter)
    if hit then
...
function TurretSeat2:sv_applyFiringImpulse(ammoData, dir, finalFirePos)
...
function TurretSeat2:sv_toggleLight(toggle)
    if not self.sv_controlsEnabled then return end
    self.network:sendToClients("cl_toggleLight", toggle)
end

function TurretSeat2:sv_SetTurretControlsEnabled(state)
    self.sv_controlsEnabled = state
    self.interactable.publicData.controlsEnabled = state
    self.network:sendToClients("cl_SetTurretControlsEnabled", state)
end

function TurretSeat2:sv_setOverrideAmmoType(id)
...
function TurretSeat2:sv_unSetOverrideAmmoType()
...
---@param part Shape
---@param player Player
function TurretSeat2:sv_OnPartFire(ammoType, ammoData, part, player) end

function TurretSeat2:sv_updateShootState(state)
    self.network:sendToClients("cl_updateShootState", state)
end



sm.MANNEDTURRET_turretSeats_clientPublicData = sm.MANNEDTURRET_turretSeats_clientPublicData or {}
function TurretSeat2:client_onCreate()
    ...
    self.lightEffect = sm.effect.createEffect("HeadLight", self.interactable)
    ...
    sm.MANNEDTURRET_turretSeats_clientPublicData[self.interactable.id] = {
        health = TurretBase.maxHealth,
        controlsEnabled = true
    }
end

function TurretSeat2:client_onDestroy()
    self.hotbar:destroy()
...
function TurretSeat2:cl_syncToLateJoiner(data)
    self:client_onClientDataUpdate(data[1], 1)
    self:client_onClientDataUpdate(data[2], 2)
end

function TurretSeat2:client_onClientDataUpdate(data, channel)
    if channel == 1 then
        self.cl_base = data
        sm.MANNEDTURRET_turretSeats_clientPublicData[self.interactable.id].base = data
    else
        self.ammoType = data
...
function TurretSeat2:client_canErase()
    print("")
    local canErase = not g_repairingTurret and sm.MANNEDTURRET_turretSeats_clientPublicData[self.interactable.id].health >= GetTurretBaseClientPublicData(self.cl_base).maxHealth and self.interactable:getSeatCharacter() == nil
...
function TurretSeat2:client_canInteract()
    ...
    local health = sm.MANNEDTURRET_turretSeats_clientPublicData[self.interactable.id].health
...
function TurretSeat2:client_onInteract(char, state)
    if not state then return end
    self:cl_seat()
end

function TurretSeat2:cl_seat()
    self.network:sendToServer("sv_seat")
    self:cl_seat_partial()
end

function TurretSeat2:cl_seat_partial()
    self.seated = true
    self.hotbar:open()
...
function TurretSeat2:cl_unSeat()
    self.network:sendToServer("sv_unSeat")
    self:cl_unSeat_graphics()
end

function TurretSeat2:cl_unSeat_graphics()
    self.shootState = ShootState.null
    self.shootTimer = 0
    self.seated = false
    self.hotbar:close()
...
function TurretSeat2:client_onAction(action, state)
...
function TurretSeat2:client_onFixedUpdate()
...
function TurretSeat2:client_onUpdate(dt)
    if not sm.exists(self.cl_base) then return end

    local speed = dt * 7.5
    self.recoil_l = math.max(self.recoil_l - speed, 0)
    self.interactable:setPoseWeight(0, sm.util.easing("easeOutCubic", self.recoil_l))

    self.recoil_r = math.max(self.recoil_r - speed, 0)
    self.interactable:setPoseWeight(1, sm.util.easing("easeOutCubic", self.recoil_r))
...
function TurretSeat2:cl_updateHotbar()
...
function TurretSeat2:cl_displayAmmoInfo()
...
function TurretSeat2:cl_shoot(args)
...
function TurretSeat2:cl_toggleLight(toggle)
    if toggle then
    ...
    self.interactable:setUvFrameIndex(toggle and 1 or 0)
end

function TurretSeat2:cl_SetTurretControlsEnabled(state)
    self.cl_controlsEnabled = state
    sm.MANNEDTURRET_turretSeats_clientPublicData[self.interactable.id].controlsEnabled = state
...
function TurretSeat2:cl_updateShootState(state)
    self.shootState = state
end



function TurretSeat2:getTurretPosition()
    local base = (self.base or self.cl_base)
    if GetTurretBaseClientPublicData(base).isLifted then
        return _G[self.baseClassName].getSeatPos({ cl_turret = self.interactable, shape = base.shape })
    end

    return self.shape.worldPosition + base.shape.velocity * 0.025
end

function TurretSeat2:getFirePos()
...
function TurretSeat2:getFirePosEnd()
    local start, _end = self:getFirePos()
    return _end
end

function TurretSeat2:isOverrideAmmoType(ammoType)
    return type(ammoType or self.ammoType) == "table"
end

function TurretSeat2:getAmmoType(parent)
    if self:isOverrideAmmoType() then
...
function TurretSeat2:getAmmoData(ammoType)
...
function TurretSeat2:canShoot(ammoType, consume)
