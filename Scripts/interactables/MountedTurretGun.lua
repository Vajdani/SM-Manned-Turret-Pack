...
function MountedTurretGun:server_onCreate()
	self.sv = {}
	self.sv.fireDelayProgress = ammoTypes[1].fireCooldown
...
function MountedTurretGun:client_canInteract()
	...
	sm.gui.setInteractionText(("<p textShadow='false' bg='gui_keybinds_bg_white' color='#444444' spacing='9'>Ammunition: %s</p>"):format(ammoTypes[self.cl.ammoType].name))
...
function MountedTurretGun:client_onInteract(char, state)
	...
	local ammoType = self.cl.ammoType < #ammoTypes and self.cl.ammoType + 1 or 1
